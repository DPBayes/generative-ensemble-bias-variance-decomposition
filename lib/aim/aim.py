...
# Source: https://github.com/ryan112358/private-pgm/blob/master/mechanisms/aim.py
# Used under the Apache-2.0 license 

# Modifications: 
# replace hdmm dependency with local code
# return measurements from AIM.run
# add option to skip synthetic data generation in AIM.run
# add option to disable print messages in AIM.run
# add option to split privacy budget with zCDP accounting


import numpy as np
import itertools
from mbi import Dataset, GraphicalModel, FactoredInference, Domain
from lib.aim.mechanism import Mechanism
from collections import defaultdict
from lib.aim.matrix import Identity
...
class AIM(Mechanism):
    def __init__(self,epsilon,delta,prng=None,rounds=None,max_model_size=80,structural_zeros={}, rho_divider=1):
        super(AIM, self).__init__(epsilon, delta, prng, rho_divider=rho_divider)
        self.rounds = rounds
    ...
    def run(self, data, W, return_syn_data=True, verbose=True):
        ...
        while not terminate:
            ...
            if verbose: print('Selected',cl,'Size',n,'Budget Used',rho_used/self.rho)
            if np.linalg.norm(w-z, 1) <= sigma*np.sqrt(2/np.pi)*n:
                if verbose: print('(!!!!!!!!!!!!!!!!!!!!!!) Reducing sigma', sigma/2)
                sigma /= 2
                epsilon *= 2
