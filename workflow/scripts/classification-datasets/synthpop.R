...
datasets = c("german-credit", "adult", "breast-cancer")

set.seed(7605870)

for (dataset in datasets){
    for (repeat_ind in 1:n_repeats){
        data_filename = sprintf("results/classification-datasets/train-test-splits/%s/repeat_%d/preprocessed-train.csv", dataset, repeat_ind - 1)
        ...
        for (syn_data_ind in 1:max_n_syn_datasets){
            output_proper = sprintf("results/classification-datasets/synthetic-data/%s/synthpop-proper_%d_%d.csv", dataset, syn_data_ind - 1, repeat_ind - 1)
